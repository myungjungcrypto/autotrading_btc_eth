...
The hot path is O(symbols * depth) because each entry or exit direction walks normalized orderbook levels once. BTC and ETH are evaluated in parallel. With depth 20 and a 50ms target loop, the CPU cost is still small on a small EC2 instance. Network latency dominates. Cascade is read through a persistent WebSocket orderbook cache; RISEx is currently polled through REST, so its response time and rate limits determine the practical lower bound. RISEx orderbook latency is logged periodically and displayed on the dashboard next to book age.
