...
Lighter market-data defaults use the read-only `wss://mainnet.zklighter.elliot.ai/stream?readonly=true` stream and the public `order_book/{MARKET_INDEX}` channel. The client sends WebSocket ping frames every 60 seconds, verifies update continuity with `begin_nonce` and `nonce`, and resubscribes with exponential reconnect backoff if a gap or disconnect occurs. Lighter live order submission is intentionally not implemented yet.
